...
void operator +=(matrix &m1, matrix &m2) {
	...
	for (unsigned int i = 0; i < m1.col; i++) {
		for (unsigned int j = 0; j < m1.row; j++)
			m1.vec[i*m1.row + j] += m2.vec[i*m2.row + j];
	}
}*/
matrix operator -(matrix &m1, matrix &m2) {
	if (!(m1.row == m2.row && m1.col == m2.col)) {
...
void operator *=(matrix &m1, const double a) {
	for (unsigned int i = 0; i < m1.col; i++) 
		for (unsigned int j = 0; j < m1.row; j++)
			m1.vec[i*m1.row + j] *= a;
}*/
...
uptriangmatrix lowtriangmatrix::transpose() {
	double temp;
	vector<double>vecc = vec;
	for (int i = 0; i < row; i++)
		for (int j = 0; j <= i; j++) {
			temp = vecc[i*row + j];
			vecc[i*row + j] = vecc[j*row + i];
			vecc[j*row + i] = temp;
		}
	uptriangmatrix m3(d, vecc);
	return m3;
}
